...
from preprocess_ISEAR import DualViewDataset
...
for run in range(num_runs):
    ...
    train_ds = torch.load('data/isear_train_dual.pt', weights_only=False)
    test_ds = torch.load('data/isear_test.pt', weights_only=False)
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            torch.save({
                ...
                'val_f1': best_val_f1,
                'epoch': epoch + 1
            }, 'results/best_contrastive_mamba_isea.pt')
            trigger_times = 0
